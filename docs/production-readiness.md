...
**Infrastructure:** Currently hosted on a DO Basic 4 GB / 2 vCPU droplet (~$24/mo) with no database backups. Needs upgrade to 8 GB / 4 vCPU + DO Managed Postgres + daily backups (~$77/mo total) for beta launch.
...
| Database | PostgreSQL 16, Drizzle ORM | Primary store (11 tables) |
...
**Race scenarios:**
- Two Telnyx webhooks arrive for the same conversation (retry): both pass the idempotency check (`webhookEvents.findFirst` at line 103 has no transaction), both insert -> **duplicate call records**
...
**Root causes:**
- No unique constraint on `calls.telnyxConversationId` (it's indexed but allows duplicates)
...
**Fix:**
1. Add `UNIQUE` constraint on `calls(telnyxConversationId)` with `ON CONFLICT UPDATE`
...
- Free/Starter plan: ~5 concurrent conversations (not enough)
- Pro plan: ~20 concurrent conversations (bare minimum for this use case)
- Enterprise: negotiable (required for multi-customer production)
...
| **ElevenLabs concurrent calls** | ~10-20 | Pro plan: ~20 | TIGHT |
...
| **DB query latency** | Unpaginated agent lists | No pagination on agents endpoint | RISK |
| **Redis** | 500-1,000 ops/min | BROKEN (see 2.1) | FAIL |
| **ElevenLabs concurrent calls** | 50-100 | Enterprise plan required | BLOCKER if not upgraded |
...
9. Upgrade to ElevenLabs Enterprise plan
...
| **ElevenLabs** | 2,000-5,000 calls/day | Enterprise plan, multiple API keys | Single key |
...
10. ElevenLabs Enterprise plan with negotiated concurrency limits
...
| `agents` | `telnyxAssistantId` | `webhooks.ts:128` | High | Add index |
| `calls` | `telnyxConversationId` | Already indexed | -- | -- |
| `appointments` | `(customerId, scheduledAt)` | `elevenlabs-webhooks.ts:636-648` (slot check during live call) | **HIGH — blocks caller on the phone** | Add composite index |
...
|----------|----------|------|
| `GET /api/agents` | `agents.ts:259-282` | Returns ALL agents for a customer. With enterprise customers creating 50+ agents, response size grows. |
...
|-------|---------------------|---------|
| `calls` | `UNIQUE(telnyx_conversation_id)` | Prevent duplicate call records from webhook retries |
...
**ElevenLabs Concurrency Limits (important for capacity planning):**
- Free/Starter plans: ~5 concurrent conversations
- Pro plan: ~20 concurrent conversations
...
**Location:** `services/email.ts` (491 lines)

| Aspect | Status | Detail |
|--------|--------|--------|
| Graceful degradation | Good | Missing API key returns 'skipped' |
| HTML sanitization | Good | `escapeHtml()` for user-provided content |
| Retry | **None** | Email failures are caught and logged, not retried |
...
|----------|-------|----------|-----|
| **Critical** | Admin fallback secret: `'voiceforge-admin-2026'` | `admin.ts:41` | Remove fallback, require env var |
| **Critical** | Admin accepts `?token=` query param | `admin.ts:40` | Header only (`X-Admin-Token`) |
| **High** | No rate limiting on admin routes | `admin.ts` | Add `authRateLimiter` (10/min) |
| **High** | Hardcoded IBAN | `registration.ts:32` | Env var |
| **High** | No CSP header | `index.ts`, `nginx/` | Add Content-Security-Policy |
| **Medium** | Rate limiter fails open on Redis failure | `rate-limit.ts:137` | Fail-closed for auth endpoints |
| **Medium** | SHA-256 for password hashing (registrations) | Registration service | Use bcrypt or argon2 |
| **Medium** | No encryption key rotation mechanism | `encryption.ts` | Document rotation procedure, add key version |
...
| Issue | Detail |
|-------|--------|
| **No CI/CD** | No GitHub Actions, no automated tests/lint before deploy |
...
Upgrade the current droplet and offload PostgreSQL to a managed service.

| Component | Change | Cost |
|-----------|--------|------|
| **Droplet** | Upgrade from Basic 4 GB / 2 vCPU to **Basic 8 GB / 4 vCPU** | ~$48/mo |
| **DO Daily Backups** | Enable automatic daily droplet backups (30% of droplet price) | ~$14/mo |
| **DO Managed Postgres** | 1 GB single node — auto daily backups, point-in-time recovery, SSL enforced | $15/mo |
| **Total** | | **~$77/mo** |

**Why this works:**
- Removing PostgreSQL from Docker frees ~1.5 GB RAM and 0.5 CPU for the API/Redis/Worker
- Managed Postgres gives you automatic daily backups and point-in-time recovery with no cron setup
- 8 GB / 4 vCPU handles 10 customers with 20 concurrent calls comfortably (after code fixes)
...
7. Resize droplet to 8 GB / 4 vCPU (requires brief downtime for droplet power-off)
8. Adjust Docker Compose container memory limits:
   - API: 512 MB -> 1 GB
   - Redis: 512 MB -> 512 MB (unchanged)
   - Next.js: 512 MB -> 1 GB
...
2. **Upgrade droplet** to Basic 8 GB / 4 vCPU (~$48/mo)
3. **Provision DO Managed Postgres** ($15/mo), migrate data, remove Postgres from Docker Compose
4. **Enable DO daily backups** on the droplet (~$14/mo)
...
|---------|---------------|----------------------|
| **ElevenLabs** | Unknown (demo API key) | Pro or Enterprise plan required for 20+ concurrent conversations. Confirm concurrency limit with ElevenLabs support. |
...
**Estimated effort: 3-4 days (code) + 1 day (infrastructure) + 1-2 days (legal/business)**
...
- [ ] **Fix call record duplication** — add `UNIQUE` on `calls(telnyxConversationId)`, use `ON CONFLICT UPDATE` in both Telnyx and ElevenLabs webhook handlers, wrap idempotency check + insert in transaction (`webhooks.ts:103-158`, `elevenlabs-webhooks.ts:199-234`)
...
- [ ] **Add rate limiting to admin routes** (`admin.ts`)
- [ ] **Add missing DB indexes** — `agents.phoneNumber` (pre-call 1s deadline), `appointments(customerId, scheduledAt)` (booking during live call)
- [ ] **Move IBAN to env var** (`registration.ts`)

**Infrastructure (see Section 11 for details):**
- [ ] **Upgrade droplet** to Basic 8 GB / 4 vCPU (~$48/mo)
...
- [ ] **Enable DO daily backups** on droplet (~$14/mo)
...
- [ ] **Verify ElevenLabs plan** supports required concurrent conversation count
...
- [ ] **Add remaining DB indexes** (`customers.stripeCustomerId`, `agents.telnyxAssistantId`)
...
- [ ] Move business hours to per-agent DB config
...
| ElevenLabs | Concurrent conversations | 5-20 (plan-dependent) | Callers get busy signal |
...
|-------|----------|---------|-----------|
| Admin secret fallback | `routes/admin.ts:41` | `'voiceforge-admin-2026'` | Required env var (no fallback) |
| IBAN | `routes/registration.ts:32` | Hardcoded bank details | Env var |
| Email FROM address | `services/email.ts:12` | `'VoiceForge AI <noreply@salimov.ai>'` | Env var |
| Plan pricing in email | `services/email.ts:273-277` | Fixed EUR values | DB or env var |
| Business hours | `routes/elevenlabs-webhooks.ts` | 09:00-17:00 hardcoded | Per-agent DB config |
| ElevenLabs SIP endpoint | `services/telnyx.ts:435` | `sip.rtc.elevenlabs.io` | Env var |
| Telnyx SIP IP whitelist | `services/telnyx.ts:869-874` | Hardcoded IPs | Env var |
...
|----------|------|----------|
| `routes/agents.ts` | `totalCalls: 0` hardcoded — should be COUNT(*) from calls table | Medium |
| `routes/elevenlabs-webhooks.ts` | Business hours hardcoded in multiple places | Medium |
...
*Last updated: 2026-03-18. Capacity estimates based on real-world use case: medium business with 4 agents handling 20 concurrent calls. Infrastructure recommendations based on current DO Basic 4 GB / 2 vCPU droplet (FRA1) running demo at voiceforge.salimov.ai.*